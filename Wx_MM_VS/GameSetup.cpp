...
GameSetup::GameSetup(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
	...
	std::ifstream* file = new std::ifstream("GameModeData.dat");
	...
	ColumnChoice->SetSelection(GameMode.numberOfColumns/2 - 2);
	...
	RepChoice->SetValue(GameMode.isRepeat);
	...
	DigitChoice->SetSelection(GameMode.numberOfColours/2 - 4);
...
{
	std::ofstream file("GameModeData.dat");
	file.write((char*)&GameMode, sizeof(GameMode));
	file.close();
    Close();
}
